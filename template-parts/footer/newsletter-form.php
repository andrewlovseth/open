<div class="newsletter-form">
    <div class="newsletter-form-container">
        ...
        <script type="text/javascript">
            /**
             * On-Demand reCAPTCHA v3 Implementation for Newsletter Form
             * 
             * Features:
             * - Loads reCAPTCHA only on first interaction with email field or when form nears viewport (600px)
             * - Pre-warms token after loading and caches for 110 seconds
             * - Submit gate validates token freshness and refreshes if needed
             * - Falls back gracefully if reCAPTCHA fails to load
             * - Prevents duplicate script loading with idempotency guards
             * - Includes resource hints for performance
             * 
             * Performance Benefits:
             * - Eliminates reCAPTCHA blocking LCP on initial paint
             * - Reduces initial page load time
             * - Maintains security with pre-warmed tokens
             */
            ...
            // Load reCAPTCHA v3 on demand
            window.loadRecaptchaOnDemand = function() {
                if (window.recaptchaState.loading || window.recaptchaState.ready || typeof grecaptcha !== 'undefined') {
                    return Promise.resolve();
                }

                window.recaptchaState.loading = true;
                window.recaptchaState.siteKey = window.getRecaptchaSiteKey();

                return new Promise(function(resolve, reject) {
                    // Check if we're in a secure context (HTTPS)
                    if (location.protocol !== 'https:' && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
                        console.warn('reCAPTCHA requires HTTPS. Form will work without verification.');
                        window.recaptchaState.loading = false;
                    ...
                    script.type = 'text/javascript';
                    script.src = 'https://www.google.com/recaptcha/api.js?render=' + window.recaptchaState.siteKey;
                    script.async = true;
                    script.defer = true;

                    var timeout = setTimeout(function() {
                        console.warn('reCAPTCHA loading timed out.');
                        window.recaptchaState.loading = false;
                        reject('timeout');
                    }, 5000);
                    ...
                    script.onload = function() {
                        ...
                        window.recaptchaState.loading = false;
                        window.recaptchaState.ready = true;
                        console.log('reCAPTCHA v3 loaded on demand');
                        
                        // Pre-warm token after loading
                        window.prewarmRecaptchaToken().then(resolve).catch(reject);
                    };
            ...
            // Pre-warm reCAPTCHA token
            window.prewarmRecaptchaToken = function() {
                if (!window.recaptchaState.ready || typeof grecaptcha === 'undefined') {
                    return Promise.reject('reCAPTCHA not ready');
                }

                return new Promise(function(resolve, reject) {
                    grecaptcha.ready(function() {
                        grecaptcha.execute(window.recaptchaState.siteKey, {action: 'newsletter'})
                        .then(function(token) {
                            window.recaptchaState.token = token;
                            window.recaptchaState.tokenTimestamp = Date.now();
                            ...
                            if (!responseElement) {
                                ...
                                document.getElementById('_form_7_').appendChild(responseElement);
                            }
                            responseElement.value = token;
                            
                            console.log('reCAPTCHA token pre-warmed');
                            resolve(token);
                        })
            ...
            // Check if token is fresh (within 110 seconds)
            window.isRecaptchaTokenFresh = function() {
                if (!window.recaptchaState.token || !window.recaptchaState.tokenTimestamp) {
                    return false;
                }
                return (Date.now() - window.recaptchaState.tokenTimestamp) < 110000;
            };

            // Get fresh token for form submission
            window.getFreshRecaptchaToken = function() {
                return new Promise(function(resolve, reject) {
                    // If token is fresh, use it
                    if (window.isRecaptchaTokenFresh()) {
                        resolve(window.recaptchaState.token);
                        return;
                    }

                    // If reCAPTCHA is ready, get new token
                    if (window.recaptchaState.ready && typeof grecaptcha !== 'undefined') {
                        window.prewarmRecaptchaToken().then(resolve).catch(reject);
                        return;
                    }

                    // If reCAPTCHA not loaded, try to load it
                    window.loadRecaptchaOnDemand().then(function() {
                        resolve(window.recaptchaState.token);
                    }).catch(reject);
                });
            };
                        ...
                        if (validate_form(e)) {
                            ...
                            if (submitButton) {
                                ...
                                submitButton.classList.add('processing');
                                    ...
                                    // Submit with fresh reCAPTCHA token or without if not available
                            var submitTimeout = setTimeout(function() {
                                console.warn('reCAPTCHA token timeout, submitting without verification');
                                submitFormWithoutRecaptcha();
                            }, 3000);

                            window.getFreshRecaptchaToken()
                            .then(function(token) {
                                ...
                                // Token is already in the hidden input, no need to add manually
                            ...
                            .catch(function(error) {
                                clearTimeout(submitTimeout);
                                console.warn('reCAPTCHA failed, submitting without verification:', error);
                                submitFormWithoutRecaptcha();
                            });
            ...
            // Setup on-demand reCAPTCHA loading triggers
            (function() {
                var emailInput = document.getElementById('email');
                var form = document.getElementById('_form_7_');
                ...
                // Trigger reCAPTCHA loading on first interaction with email field
                function triggerRecaptchaLoad() {
                    if (window.recaptchaState.triggered) {
                        return;
                    }
                    window.recaptchaState.triggered = true;
                    console.log('Triggering reCAPTCHA load on demand');
                    window.loadRecaptchaOnDemand().catch(function(error) {
                        console.warn('Failed to load reCAPTCHA on demand:', error);
                    });
                }

                // Add event listeners for email input interactions
                emailInput.addEventListener('focus', triggerRecaptchaLoad, {once: true});
                emailInput.addEventListener('keydown', triggerRecaptchaLoad, {once: true});
                emailInput.addEventListener('pointerdown', triggerRecaptchaLoad, {once: true});
                emailInput.addEventListener('touchstart', triggerRecaptchaLoad, {once: true});

                // Setup intersection observer for viewport proximity
                if ('IntersectionObserver' in window) {
                    ...
                    checkFormVisibility(); // Check initial position
                }
            })();
        </script>
